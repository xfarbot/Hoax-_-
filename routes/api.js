...
var kuhong = db.get('ojan'); // jan diubah
	...
	console.log('WELCOME TO XFar API!') // boleh diubah
}

var creatorList = ['@xfar05']; // Nama Lu Ngab
var creator = creatorList[Math.floor(Math.random() * creatorList.length)]; // Ini jan diubah
var key = 'xfarkey' // Apikey Lu Ngab
...
loghandler = {
    ...
    invalidKey: {
        ...
        creator: `${creator}`,
        code: 406,
        message: `Apikey tidak ditemukan! Silahkan kontak Owner untuk dapatkan Apikey wa.me/62895619083555`
    },
    invalidLink: {
        status: false,
        creator: `${creator}`,
...
if (!req.query.desc) return res.json({ status: 404, error: 'masukkan param desc'})
if (!req.query.bgurl) return res.json({ status: 404, error: 'masukkan param bgurl'})
const canvacard = require("canvacard");
const welcomer = await new canvacard.Welcomer()
                    ...
                    welcomer.build() 
                    .then(data => {
                const base64 = `${data.toBuffer().toString('base64')}`
...
if (!req.query.bgurl) return res.json({ status: 404, error: 'masukkan param bgurl'})
const canvacard = require("canvacard");
const welcomer = await new canvacard.Leaver()
                    .setUsername(req.query.name)
                    .setDiscriminator(req.query.mem)
                    .setGuildName(req.query.gcname)
                    .setDescription(req.query.desc)
